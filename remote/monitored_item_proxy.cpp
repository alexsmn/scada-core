...
void MonitoredItemProxy::SubscribeData(scada::DataChangeHandler handler) {
  assert(handler);
  assert(!subscribed_);
...
void MonitoredItemProxy::SubscribeEvents(scada::EventHandler handler) {
  assert(handler);
  assert(!subscribed_);
  assert(state_ == State::DELETED);
...
void MonitoredItemProxy::CreateStub() {
  ...
  sender_->Request(request, [weak_ptr = weak_from_this()](
                                const protocol::Response& response) {
    if (auto ptr = weak_ptr.lock()) {
      ptr->OnCreateMonitoredItemResult(
